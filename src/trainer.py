...
class Trainer:
    def __init__(
        ...
        if self._load_buffer:
            self.buffer = MultiEnvReplayBuffer.load(self._load_buffer, self._reset_rnn_states)
            logger.info("Loaded buffer from %s (%d transitions)", self._load_buffer, len(self.buffer))
        else:
            self.buffer = MultiEnvReplayBuffer(
                num_envs=1,
    ...
    def train(self):
        ...
        while global_step < self.total_env_steps:
            ...
            # The stored RNN state is the state *before* processing obs; the buffer
            # tracks is_first and arrival alignment internally. Cast to the buffer's
            # fp16 on GPU: numpy's fp32->fp16 assignment cast is a scalar loop.
            self.buffer.push(
                obs[None],
                np.array([action], dtype=np.uint8),
                np.array([reward], dtype=np.float32),
                # Only true termination marks a terminal state; a truncated episode's
                # final state is not terminal and keeps cont target gamma.
                np.array([terminated]),
                prev_rssm_state["deter"].to(torch.float16).cpu().numpy(),
                prev_rssm_state["stoch"].to(torch.float16).cpu().numpy(),
                is_first=np.array([is_first]),
            )
            is_first = False

            if done:
                # Push the terminal state itself to the buffer
                self.buffer.push(
                    next_obs[None],
                    np.array([0], dtype=np.uint8),
                    np.array([0.0], dtype=np.float32),
                    np.array([False]),
                    rssm_state["deter"].to(torch.float16).cpu().numpy(),
                    rssm_state["stoch"].to(torch.float16).cpu().numpy(),
            ...
            if global_step >= self.warmup_steps and len(self.buffer) >= self.batch_size * (
                ...
                if train_step_counter % self.train_every == 0:
                    ...
                    # Cast to the buffer's fp16 on GPU: halves the device-to-host copy
                    # and lets update_rnn_states skip its CPU cast. Safe: deter is
                    # tanh-bounded, stoch is one-hot.
                    deter = rssm_out["deter_state"].detach().to(torch.float16)
                    stoch = rssm_out["posterior"]["sample"].detach().to(torch.float16)
