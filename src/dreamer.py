...
class Dreamer(nn.Module):
    ...
    def wm_forward_step(self, batch, burn_in_steps: int) -> torch.Tensor:
        ...
        with autocast(device_type=self.amp_device, dtype=self.amp_dtype, enabled=self.use_amp):
            ...
            discount_target = continuation_target(dones, is_first, self.gae_gamma).unsqueeze(-1)
...
def continuation_target(dones, is_first, gamma: float):
    """Target for the discount/cont head: gamma everywhere except true terminals.

    The buffer sets done=True at is_first slots only to isolate episodes in the
    replay GAE; the spawn state itself is not terminal, so it must keep target gamma.
    """
    return (1.0 - (dones.bool() & ~is_first.bool()).float()) * gamma
