...
def get_optimizer_param_groups(model, exclude_keywords=("out", "head", "dyn_to_gru")):
    """Split params into (muon, other): >=2D weights go to Muon unless a name segment is excluded."""
    muon_params = []
    other_params = []
    ...
    for name, param in model.named_parameters():
        ...
        parts = name.split(".")
        should_exclude = any(kw in parts for kw in exclude_keywords)
