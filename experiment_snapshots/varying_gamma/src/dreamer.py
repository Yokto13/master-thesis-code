...
from utils import dict_apply, exponential_decay_scheduler, get_post_burn_in, unimix
...
class Dreamer(nn.Module):
    def __init__(
        ...
        self.predict_continues = predict_continues

        gamma_decay_requested = gamma_start is not None or gamma_end is not None
        if gamma_decay_requested:
            assert (
                gamma_start is not None and gamma_end is not None
            ), "gamma_start and gamma_end must both be set to enable gamma decay"
            assert (
                0.0 <= gamma_start < 1.0 and 0.0 <= gamma_end < 1.0
            ), "gamma_start and gamma_end must be in [0, 1) for reverse exponential decay"
            assert (
                total_training_steps is not None and total_training_steps > 0
            ), "total_training_steps must be passed from the Trainer to enable gamma decay"
            assert predict_continues, (
                "gamma decay is only valid when predict_continues=True — otherwise the "
                "discount head would chase a moving target"
            )
            self._gamma_scheduler = exponential_decay_scheduler(
                decay_period=total_training_steps,
                initial_value=gamma_start,
                final_value=gamma_end,
                reverse=True,
            )
        else:
    ...
    def current_gamma(self, step: int | None) -> float:
        if self._gamma_scheduler is None or step is None:
            return self.gae_gamma
        return self._gamma_scheduler(step)

    def train_step(
        self, batch, burn_in_steps: int, train_only_wm: bool = False, step: int | None = None
    ) -> tuple[dict, dict]:
        gamma = self.current_gamma(step)
        ...
        if train_only_wm:
            ...
            ac_losses, ac_metrics = self.ac_forward_step(rssm_out, rewards, dones, gamma=gamma)
        ...
        return rssm_out, {**wm_metrics, **ac_metrics, **backward_metrics, **losses, "current_gamma": gamma}
    ...
    def wm_forward_step(self, batch, burn_in_steps: int) -> torch.Tensor:
        ...
        with autocast(device_type=self.amp_device, dtype=self.amp_dtype, enabled=self.use_amp):
            ...
            if not self.predict_continues:
                discount_target = discount_target * self.gae_gamma
    ...
    def ac_forward_step(self, rssm_out, rb_rewards, rb_dones, gamma: float):
        loss = {}
        with autocast(device_type=self.amp_device, dtype=self.amp_dtype, enabled=self.use_amp):
            ...
            if self.predict_continues:
                weights = torch.cumprod(gamma * discounts, dim=0).detach()
            else:
        ...
        adv, returns = compute_gae(
            ...
            dones=1.0 - discounts,
            lam=self.gae_lambda,
            gamma=gamma if self.predict_continues else 1.0,
        )
