...
from monitoring import MonitoringSuite
...
class Trainer:
    def __init__(
        ...
        load_validation_buffer: str = "",
        enable_monitorch: bool = False,
    ):
        ...
        self._load_validation_buffer = load_validation_buffer
        ...
        self.dreamer: Dreamer = Dreamer(
            stoch_dim=self.stoch_dim, deter_dim=self.deter_dim, latent_dim=self.latent_dim, action_dim=self.action_dim
        ).to(self.device)
        ...
        if self._load_validation_buffer:
            self.validation_buffer = SingleEnvOnlineReplayBuffer.load(
                self._load_validation_buffer, self._reset_rnn_states
            )
            logger.info(
                "Loaded validation buffer from %s (%d transitions)",
                self._load_validation_buffer,
                len(self.validation_buffer),
            )
        else:
        ...
        self.monitoring: MonitoringSuite | None = None
        if enable_monitorch:
            logger.info("Monitoring enabled: attaching MonitoringSuite to Dreamer + optimizer")
            self.monitoring = MonitoringSuite(self.dreamer, self.dreamer.optimizer)
            ...
            if global_step >= self.warmup_steps and len(self.buffer) >= self.batch_size * (
                ...
                if train_step_counter % self.train_every == 0:
                    ...
                    rssm_out, metrics = self.dreamer.train_step(
                        batch, burn_in_steps=self.burn_in_steps, train_only_wm=self.train_only_wm
                    )
                    ...
                    if global_step - self._last_log_step >= self.log_interval and train_step_counter >= 500:
                        ...
                        if self.validation_buffer is not None:
                            self.dreamer.eval()  # Ensure model is in eval mode for validation to turn off dropout
                            accumulated_val_metrics = {k: [] for k in metric_key_map.values()}
                            for _ in range(
                                len(self.validation_buffer) // (self.batch_size * (self.seq_len + self.burn_in_steps))
                            ):
                                val_batch = self.validation_buffer.sample(
                                    self.batch_size, self.seq_len + self.burn_in_steps
                                )
                                for k, v in val_batch.items():
                                    if k == "rnn_states":
                                        for kk in ["deter", "stoch"]:
                                            val_batch[k][kk] = torch.from_numpy(v[kk]).to(
                                                self.device, non_blocking=True
                                            )
                                    else:
                                        val_batch[k] = torch.from_numpy(v).to(self.device, non_blocking=True)
                                with torch.inference_mode():
                                    _, val_metrics = self.dreamer.train_step(
                                        val_batch,
                                ...
                                for k, v in val_metrics.items():
                                    if k in metric_key_map.values():
                                        accumulated_val_metrics[k].append(v)
                            # reduce accumulated metrics and add "val_" prefix
                            for k, v in accumulated_val_metrics.items():
                                if v:
                                    log_data[f"val/{k}"] = torch.stack(v).mean().item()
                        ...
                        if self.monitoring is not None:
                            self.monitoring.log_to_wandb(global_step)
